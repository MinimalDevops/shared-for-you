...
async def download_pdf(url, pdf_name, pdf_dir, md_file):
    ...
    try:
        ...
        if os.path.exists(pdf_path):
            ...
            summarize_pdf_and_update_md(pdf_path, md_file)
            return
...
def summarize_pdf_and_update_md(pdf_path, md_file):
    ...
    # Find the line with the URL and add the content on the next line
    ...
    for i, line in enumerate(lines):
        ...
        # If the line contains a URL, add the pdf_content on the next line
        if url_pattern.search(line):
            updated_lines.append(pdf_content + '\n')
...
def process_markdown(md_path):
    ...
    for i, line in enumerate(lines):
        ...
        if url_match:
            ...
            asyncio.get_event_loop().run_until_complete(download_pdf(url, pdf_name, pdf_dir, md_path))
